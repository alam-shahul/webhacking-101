...
import GameData from '../public/data/game.json';
...
class Game extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      updatePuzzleHash: props.updatePuzzleHash,
      loading: true,
      movesLeft: 0,
    }
  }
  ...
  handleClick(e, i) {
    ...
    else if (!checkColumnValues(currentsquares[0].squares, currentsquares[0].squares[i])) {
      errorMessage = true;
      // console.log("number ya existe en la columna");
    }
    else if (!checkBlockValues(currentsquares[0].squares, currentsquares[0].squares[i])) {
      errorMessage = true;
    ...
    //Calculates how many moves are left to finish the game
      ...
      movesLeft--;
	  if (movesLeft === 0) {
		this.state.updatePuzzleHash(JSON.stringify(currentsquares))
	  }
    } 
